...
Db.prototype.deleteTables = function(){
    ...
    sequence(this).then(function(next){
        ...
        this.client.query("DROP TABLE devices", function(err){
            if(err){
                return d.reject();
            }
            return next();
...
// Db.prototype.selectUser = function(user){
//     var d = when.defer();

//     this.client.query("SELECT username, device_id " +
//                     "FROM users " +
//                     "WHERE name = $1",[user], function(err,result){
//         if(err){
//             return d.reject(err);
//         }
//         var users = {};
//         for( var i = 0; i<result.rows.length;i++){
//             if(!users.hasOwnProperty(result.rows[i].name)){
//                 users[result.rows[i].name] = {
//                     'devices': []
//                 };
//             }
//             users[result.rows[i].name].devices.push(result.rows[i].device_id);
//         }
