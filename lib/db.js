...
Db.prototype.logUsage = function(deviceId, date, socket1, socket2){
    ...
    console.log('usage values ' + socket1 + ', ' + socket2 + ' for ' + deviceId + ' on ' + date);
    var d = when.defer();
...
Db.prototype.logDisconnect = function(deviceId, date){
    var d = when.defer();

    this.client.query("INSERT INTO usagedata VALUES ($1, $2, null, null);", [deviceId, date], function(err, result){
        if (err){
            return d.reject(err);
        }
        return d.resolve();
